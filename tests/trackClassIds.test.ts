...
test('classesInMajorScope excludes disabled and out-of-scope classes', () => {
  const result = classesInMajorScope(major({ targetClassIds: ['c2'] }), [
    schoolClass({ id: 'c1' }),
    schoolClass({ id: 'c2' }),
    schoolClass({ id: 'c3', enabled: false }),
  ]);
  assert.deepEqual(
...
test('recomputeMajorsTrackClassIds leaves manually scoped quick temporary majors unchanged', () => {
  const current = [
    major({ source: 'quick', temporary: true, targetClassIds: ['c1'], items: [item('chemistry', CHEMISTRY)] }),
  ];
  const result = recomputeMajorsTrackClassIds(
    current,
    [schoolClass({ id: 'c1', track: [HISTORY, GEOGRAPHY, POLITICS] })],
    true,
  );
