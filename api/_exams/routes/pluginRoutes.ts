...
export async function handlePluginPairStart(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  if (!credentials || !PLUGIN_TOKEN_RE.test(pairToken)) {
    res.status(400).json({ ok: false, error: 'Invalid plugin credentials' });
    return;
  }
  ...
  if (existing[0]?.client_secret_hash && !equalHash(existing[0].client_secret_hash, secretHash)) {
    res.status(401).json({ ok: false, error: 'Plugin credentials rejected' });
    return;
  }
  const now = Date.now();
  await sql`
...
export async function handlePluginPairInfo(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  const binding = device
    ? {
        ...
        classTag: classLabel(payload, device.grade_id ?? '', device.class_id ?? ''),
      }
    : null;
  res.status(200).json({
    ok: true,
...
export async function handlePluginPairConfirm(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  const gradeValid = payload.grades.some((item) => item?.id === gradeId && item?.enabled !== false);
  const classValid = payload.classes.some(
    (item) => item?.id === classId && item?.gradeId === gradeId && item?.enabled !== false,
  );
...
export async function handlePluginPairStatusOrBootstrap(
  ...
  const plugin = rows[0];
  if (!plugin || !equalHash(plugin.client_secret_hash, sha256(credentials.secret))) {
    res.status(401).json({ ok: false, error: 'Plugin credentials rejected' });
    return;
  }
  ...
  const viewerBindingValid =
    !!device && !device.revoked && !device.is_management && !!device.grade_id && !!device.class_id;
...
export async function handlePluginViewerHeartbeat(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  if (
    ...
    res.status(409).json({ ok: false, error: 'Plugin is not paired with this viewer' });
    return;
  }
  const deviceRows =
