...
export function pluginCredentials(
  body: Record<string, unknown>,
): { instanceId: string; secret: string } | null {
  const instanceId = String(body.pluginInstanceId ?? "").trim();
  const secret = String(body.clientSecret ?? "").trim();
  return PLUGIN_ID_RE.test(instanceId) && PLUGIN_SECRET_RE.test(secret)
    ? { instanceId, secret }
    : null;
}
...
export function classLabel(
  ...
  const grades = Array.isArray(payload.grades)
    ? (payload.grades as Array<Record<string, unknown>>)
    : [];
...
export function actorScopeLabel(
  actor: AdminActor,
  payload: ExamPayload,
): string {
  if (
  ...
  const names = actor.scopes
    .map((scope) => {
      if (scope.type === "grade")
        return String(
          grades.find((item) => String(item.id ?? "") === scope.gradeId)
            ?.name ?? scope.gradeId,
        );
      if (scope.type === "class")
        return classLabel(payload, scope.gradeId, scope.classId).replace(
          " ",
          " · ",
        );
      return "";
...
export function resolvePluginExams(
  payload: ExamPayload,
  gradeId: string,
  classId: string,
) {
  ...
  const schedule = resolveEffectiveSchedule(
    {
      ...
      activeWeeklyPlanIdByClassId:
        payload.activeWeeklyPlanIdByClassId as Record<string, string | null>,
