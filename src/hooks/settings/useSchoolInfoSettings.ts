...
export function useSchoolInfoSettings(canEditSchool: boolean) {
  ...
  const [schoolName, setSchoolName] = useState(() => getAppSettings().exam.initialization.schoolName);
  const [province, setProvince] = useState(() => getAppSettings().exam.initialization.province);
  ...
  const saveSchoolName = async () => {
    ...
    const result = await saveExamsToServer(buildExamSaveInput({ initialization }));
