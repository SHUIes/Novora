...
import { checkForUpdate, getDeployStatus, triggerRedeploy } from '../../services/update';
...
export function useDeploymentSettings() {
  ...
  const doRedeploy = async () => {
    if (
      !(await confirmDialog({
        title: '重新部署 Novora',
        message: '将从 GitHub 拉取最新代码并重新构建，约需 1-3 分钟。完成后刷新页面即可使用新版本。',
        tone: 'warning',
        confirmLabel: '开始部署',
      }))
    )
    ...
    if (r.ok) {
      ...
      const message = r.code === 'NO_HOOK' ? '未配置部署钩子（VERCEL_DEPLOY_HOOK_URL）' : r.error || '触发失败';
