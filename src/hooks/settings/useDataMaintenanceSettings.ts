...
import { getAppSettings, updateExamSettings, APP_SETTINGS_KEY } from '../../utils/appSettings';
import {
  saveExamsToServer,
  getCloudSnapshot,
...
export function useDataMaintenanceSettings(canResetDatabase: boolean) {
  ...
  const resetCloudData = async () => {
    if (!canResetDatabase || resetPhrase !== '重置数据库' || !resetCategories.length) {
  ...
  const updateDemoData = async (enable: boolean) => {
    ...
    const majors = enable
      ? [...exam.majors.filter((item) => !item.id.startsWith('demo_v2_')), demoMajor]
      : exam.majors.filter((item) => !item.id.startsWith('demo_v2_'));
    const weeklyPlansNext = enable
      ? [...exam.weeklyPlans.filter((item) => !item.id.startsWith('demo_v2_')), demoPlan]
    ...
    const activeWeeklyPlanIdByClassId = {
      ...
      [demoPlan.classId]: enable
        ? demoPlan.id
        : (weeklyPlansNext.find((item) => item.classId === demoPlan.classId)?.id ?? null),
    };
    ...
    try {
      ...
      notify('error', error instanceof Error ? error.message : '演示数据操作失败');
    } finally {
      setDemoBusy(false);
    }
  };
