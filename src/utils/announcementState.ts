...
export function isAnnouncementRead(a: Announcement): boolean {
  return reads()[String(a.id)] === Number(a.updated_at);
}
export function markAnnouncementRead(a: Announcement): void {
  try {
    const r = reads();
    r[String(a.id)] = Number(a.updated_at);
