...
async function put(key: string, value: unknown): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    tx.objectStore('records').put({ key, value, updatedAt: Date.now() } satisfies RecordValue);
    tx.oncomplete = () => resolve();
    tx.onerror = () => reject(tx.error);
  });
  db.close();
}
export async function mirrorAppSettings(value: unknown): Promise<void> {
  try {
    await put('app-settings-v1', value);
  } catch {
    /* localStorage remains fallback */
  }
}
export async function recordSyncConflict(conflictCount: number, local: unknown, remote: unknown): Promise<void> {
  try {
    await put(`conflict-${Date.now()}`, { conflictCount, local, remote });
  } catch {
    /* conflict remains in pending outbox */
  }
}
