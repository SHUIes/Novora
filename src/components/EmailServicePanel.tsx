...
const EmailServicePanel = forwardRef<EmailServicePanelHandle, EmailServicePanelProps>(function EmailServicePanel(
  { canEditSettings, showPolicy = false, canEditPolicy = false, onSaved },
  ref,
) {
  ...
  const test = async () => {
    ...
    try {
      ...
      setErr(cause instanceof Error ? cause.message : '测试发送失败');
    } finally {
      setTesting(false);
    }
  };
  ...
  const clear = async () => {
    ...
    try {
      ...
      setErr(cause instanceof Error ? cause.message : '停用失败');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="set-fieldset">
      {enabled && (
        <div className="set-status__list">
          <li>
            <span>服务状态</span>
            <b>已启用</b>
          </li>
      ...
      <div className="set-row">
        ...
        <input
          ...
          placeholder="如 noreply@school.edu.cn"
        />
      </div>
      <div className="set-row">
      ...
      <div className="set-row">
        ...
        <input
          ...
          placeholder="留空则不限（以已绑定邮箱为准）；多个用逗号分隔"
        />
      </div>
      <div className="set-row">
        ...
        <div className="set-row__inline">
          ...
          />
          <button className="set-btn" disabled={!canEditSettings || testing} onClick={() => void test()}>
            {testing ? '发送中…' : '发送测试邮件'}
          </button>
        </div>
      </div>
        ...
        <div className="set-fieldset__policy">
          ...
          <div className="set-policy-options">
            {POLICY_OPTIONS.map((option) => (
              <label
                key={option.value}
                className={'set-policy-option' + (initBindPolicy === option.value ? ' is-active' : '')}
              >
                <input
                  type="radio"
                  name="init-bind-policy"
                  checked={initBindPolicy === option.value}
                  disabled={!canEditPolicy}
                  onChange={() => {
                    setInitBindPolicy(option.value);
                    setErr('');
                  }}
                />
                <span>
      ...
      <div className="set-actions">
        <button className="set-btn set-btn--primary" disabled={!canEditSettings || saving} onClick={() => void save()}>
          {saving ? '保存中…' : '保存并启用'}
        </button>
        {enabled && canEditSettings && (
          <button className="set-btn set-btn--danger" disabled={saving} onClick={() => void clear()}>
            停用并清空
          </button>
        )}
      </div>
