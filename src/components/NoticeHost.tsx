...
export default function NoticeHost() {
  ...
  useEffect(() => {
    const receive = (event: Event) => {
      ...
      window.setTimeout(
        () => setItems((current) => current.filter((item) => item.id !== detail.id || item.updatedAt !== updatedAt)),
    ...
    return () => {
      ...
      window.removeEventListener(NOTICE_DISMISS_EVENT, dismiss);
    };
  }, []);
  return (
    <div className="notice-host" aria-live="polite">
      {items.map((item) => {
        ...
        return (
          <article
            className={`notice-toast is-${item.tone}${isQueue ? ' is-queue' : ''}`}
            key={item.id}
          ...
          >
            ...
            <div>
              ...
              <p>{item.message}</p>
              {isQueue && <span className="notice-toast__queue-bar" aria-hidden="true" />}
            </div>
            <button
              aria-label="关闭提醒"
