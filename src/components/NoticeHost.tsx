...
export default function NoticeHost() {
      ...
      setItems((current) => {
        ...
        if (exists)
          return current.map((item) =>
            item.id === detail.id ? { ...item, ...detail, updatedAt } : item,
          );
        return [...current.slice(-3), { ...detail, updatedAt }];
      });
      window.setTimeout(
        () =>
          setItems((current) =>
            current.filter(
              (item) => item.id !== detail.id || item.updatedAt !== updatedAt,
            ),
          ),
        detail.durationMs ?? (detail.tone === 'error' ? 6500 : 4200),
      );
    };
  ...
  return (
    <div className="notice-host" aria-live="polite">
      {items.map((item) => {
        const { Icon, label } = META[item.tone];
        return (
          <article
            className={`notice-toast is-${item.tone}${item.variant === 'queue' ? ' is-queue' : ''}`}
            key={item.id}
          ...
          >
            ...
            <div>
              ...
              <p>{item.message}</p>
              {item.variant === 'queue' && (
                <span className="notice-toast__queue-bar" aria-hidden="true" />
              )}
            </div>
            <button
              aria-label="关闭提醒"
