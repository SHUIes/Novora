    ...
    <>
      ...
      >
        <div className="admin-modal admin-modal--wide admin-modal--workflow" onClick={(e) => e.stopPropagation()}>
          <h2 className="admin-modal__title admin-workflow-head">{editing.id ? '编辑周测' : '添加周测'}</h2>
          <AdminWorkflowClose
            onClick={() => {
              setWeeklyTimeFlowOpen(false);
              setEditing(null);
          ...
          <div className="admin-workflow-layout">
            <AdminWizardSteps
              ...
              summary={
                <>
                  <span>当前周测</span>
                  <strong>{editing.name || '尚未选择科目'}</strong>
                  <span>
                    {WEEKDAY_LABEL[editing.weekday]} · {editing.startTime || '--:--'} - {editing.endTime || '--:--'}
                  </span>
                </>
              }
            />
                ...
                <div className="admin-workflow-pane">
                  <label className="admin-label">
                    ...
                    <InlineSelect
                      ...
                      ariaLabel="选择周测科目"
                      value={
                        customWeeklySubjectActive || (editing.name && !COMMON_WEEKLY_SUBJECTS.includes(editing.name))
                          ? CUSTOM_WEEKLY_SUBJECT
                          : editing.name
                      }
                      placeholder="选择常用科目"
                      options={[
                        { value: '', label: '选择常用科目' },
                        ...COMMON_WEEKLY_SUBJECTS.map((subject) => ({
                          value: subject,
                          label: (
                            <>
                    ...
                    />
                    {(customWeeklySubjectActive ||
                      (editing.name && !COMMON_WEEKLY_SUBJECTS.includes(editing.name))) && (
                      <input
                ...
                <div className="admin-workflow-pane admin-workflow-pane--two-column">
                  <label className="admin-label">
                    ...
                    <InlineSelect
                      ...
                      onChange={(value) => setEditing((p) => p && { ...p, weekday: Number(value) as IsoWeekday })}
                  ...
                  {planWeekMode === 'ab' && (
                    <label className="admin-label">
                      ...
                      <InlineSelect
                        ...
                        onChange={(value) => setEditing((p) => p && { ...p, weekType: value as WeeklyWeekType })}
                        options={[
              ...
              {itemWizardStep === 2 && (
                <div className="admin-workflow-pane">
                  <div className="admin-workflow-review">
                    <span>
                      考试科目<strong>{editing.name || '未选择'}</strong>
                    </span>
                    <span>
                      进行时间
                      <strong>
                        {WEEKDAY_LABEL[editing.weekday]} {editing.startTime} - {editing.endTime}
                        {editing.endNextDay ? '（次日）' : ''}
                      </strong>
                    </span>
                    <span>
                      适用周次
                    ...
                    <span>
                      地点备注<strong>{editing.location || '无'}</strong>
                    </span>
                    <span>
                      启用状态<strong>{editing.enabled ? '启用' : '停用'}</strong>
                    </span>
                  </div>
                </div>
              )}
          ...
          <div className="admin-modal__actions">
            ...
            {itemWizardStep < 2 ? (
              ...
              <button className="admin-btn admin-btn--primary admin-workflow-actions-spacer" onClick={commitItemModal}>
                确认并保存
              </button>
            )}
          </div>
