...
export default function DeletePlanModals({
  ...
  return (
    <>
      {deletePlanOpen && (
        <AdminModalPortal className="admin-modal-overlay" {...backdropProps(() => setDeletePlanOpen(false))}>
          <div className="admin-modal" onClick={(e) => e.stopPropagation()}>
            <h2 className="admin-modal__title">删除周测计划</h2>
            <p className="admin-modal__body">
              确定删除「{activePlan.name}」及其全部 {itemsCount} 条周测？删除后可在页面顶部立即撤销。
            </p>
            <div className="admin-modal__actions">
              <button className="admin-btn admin-btn--danger" onClick={removePlan}>
                删除
              </button>
              <button className="admin-btn" onClick={() => setDeletePlanOpen(false)}>
                取消
              </button>
            </div>
          </div>
        ...
        >
          ...
          >
            ...
            <div className="admin-workflow-layout">
              <AdminWizardSteps
                active={batchDeleteStep}
                steps={[
                  { label: '选择计划', hint: '按年级筛选并勾选' },
                  { label: '确认删除', hint: '核对影响范围' },
                ]}
                summary={
                  <>
                    <span>已选择</span>
                    <strong>{batchDeletePlanIds.length} 个计划</strong>
                    <span>删除后可整批撤销</span>
                  </>
                }
              />
              <div className="admin-workflow-content" key={batchDeleteStep}>
                {batchDeleteStep === 0 && (
                  <div className="admin-workflow-pane">
                    <p className="admin-modal__body">按年级和班级选择要删除的具体计划。</p>
                    <ClassMultiPicker
                ...
                {batchDeleteStep === 1 && (
                  <div className="admin-workflow-pane">
                    <div className="admin-workflow-review">
                      <span>
                        删除数量<strong>{batchDeletePlanIds.length} 个周测计划</strong>
                      </span>
                      <span>
                        删除后处理<strong>自动切换各班剩余计划；无剩余则清空</strong>
                      </span>
                      <span>
                        恢复方式<strong>页面顶部支持整批撤销</strong>
                      </span>
                    </div>
                    <p className="admin-major-card__hint">请确认所选年级和班级无误后再删除。</p>
                  </div>
                )}
              </div>
            </div>
            <div className="admin-modal__actions">
              <button
                className="admin-btn"
                onClick={() => {
                  if (batchDeleteStep) setBatchDeleteStep(0);
                  else {
                    setBatchDeleteOpen(false);
                    setBatchDeletePlanIds([]);
                  }
              ...
              >
                {batchDeleteStep ? '上一步' : '取消'}
              </button>
              {batchDeleteStep === 0 ? (
                <button
                  className="admin-btn admin-btn--primary admin-workflow-actions-spacer"
                  disabled={!batchDeletePlanIds.length}
                  onClick={() => setBatchDeleteStep(1)}
                >
                  下一步，确认范围
                </button>
              ) : (
                <button
                  className="admin-btn admin-btn--danger admin-workflow-actions-spacer"
      ...
      {deleteTarget && (
        <AdminModalPortal className="admin-modal-overlay" {...backdropProps(() => setDeleteTarget(null))}>
          <div className="admin-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="admin-modal__actions">
              <button className="admin-btn admin-btn--danger" onClick={() => removeItem(deleteTarget)}>
                删除
              </button>
