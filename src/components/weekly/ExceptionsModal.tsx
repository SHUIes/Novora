...
export default function ExceptionsModal({
  ...
  return (
    <AdminModalPortal className="admin-modal-overlay" {...backdropProps(() => setExceptionsOpen(false))}>
      <div className="admin-modal admin-modal--wide" onClick={(e) => e.stopPropagation()}>
        <h2 className="admin-modal__title">例外日期管理</h2>
        <p className="admin-modal__body">
          整日排除的日期当天完全不生成周测；下方“单次调整”是“取消本次 / 临时调课 / 本周仍然进行”产生的记录，可在此撤销。
        </p>
          ...
          <section>
            ...
            <label className="admin-toggle-label">
              <input
                ...
                onChange={(e) =>
                  onSavePlans(
                    weeklyPlans.map((p) =>
                      p.id === activePlan.id ? { ...p, excludeOfficialHolidays: e.target.checked } : p,
                    ),
                    activePlan.id,
                    selectedClassId,
                    true,
            ...
            {activePlan.excludeOfficialHolidays && (
              <p className="admin-major-card__hint weekly-holiday-summary">
                {OFFICIAL_HOLIDAYS.map((item) => `${item.name} ${item.start.slice(5)}~${item.end.slice(5)}`).join(
                  ' · ',
                )}
              </p>
            )}
            <div className="admin-form">
              ...
              <button className="admin-btn admin-btn--primary" onClick={addExcludedDate}>
                添加排除日
              </button>
            </div>
                  ...
                  <li className="admin-item" key={ov.id}>
                    <div className="admin-item__info">
                      <span className="admin-item__name">
                        {ov.date} ·{' '}
                        {ov.action === 'cancel' ? '取消本次' : ov.forceRunDuringMajorExam ? '强制仍然进行' : '临时调课'}
                        {ov.name ? `（${ov.name}）` : ''}
                      </span>
                      {ov.reason && (
                        <div className="admin-item__times" style={{ opacity: 0.7 }}>
                          {ov.reason}
                        </div>
                      )}
                    </div>
                    <div className="admin-item__actions">
                      <button className="admin-item-btn admin-item-btn--delete" onClick={() => removeOverride(ov.id)}>
                        撤销
                      </button>
                    </div>
                  </li>
        ...
        <div className="admin-form-actions">
          <button className="admin-btn admin-btn--primary" onClick={() => setExceptionsOpen(false)}>
            完成
          </button>
        </div>
      </div>
