...
export default function AccountEmailBinding() {
  ...
  const send = async () => {
    ...
    try {
      ...
      setErr(cause instanceof Error ? cause.message : '验证码发送失败');
    } finally {
      setBusy(false);
    }
  };
  ...
  const confirm = async () => {
    ...
    try {
      ...
      setDraft('');
      setCode('');
      setMsg('邮箱绑定成功，之后可用邮箱验证码登录');
    } catch (cause) {
      setErr(cause instanceof Error ? cause.message : '邮箱绑定失败');
    } finally {
      setBusy(false);
    }
  };
  ...
  const unbind = async () => {
    const ok = await confirmDialog({
      title: '解绑邮箱',
      message: '确定解绑当前邮箱吗？解绑后将无法使用邮箱验证码登录。',
      tone: 'danger',
      confirmLabel: '解绑',
    });
    if (!ok) return;
    ...
    try {
      ...
      setErr(cause instanceof Error ? cause.message : '解绑失败');
    } finally {
      setBusy(false);
    }
  };
